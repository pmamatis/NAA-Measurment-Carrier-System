...
def on_message(client, userdata, message):
    ...
    ba.reverse()
    hex_value = (ba.hex()[::-1])
    #Messdatenformat: version, sekunden, µsekunden, Spannung, Strom, Leistung. Alle Daten haben 4 Byte (8 zeichen)
    version =    hex_value[0*8:1*8][::-1]

    # convert hex to int
    sekunden =  int(hex_value[1*8:2*8][::-1],16)
    u_sekunden = int(hex_value[2*8:3*8][::-1] ,16)

    #convert hex to float
    voltage =    struct.unpack('!f', bytes.fromhex((hex_value[3*8:4*8][::-1])))[0]
    current =     struct.unpack('!f', bytes.fromhex( hex_value[4*8:5*8][::-1]))[0]
    power =       struct.unpack('!f', bytes.fromhex( hex_value[5*8:6*8][::-1]))[0]
    ...
    # get subtopic
    rail_index = 0
    for rail in rail_topics:
        if rail in incoming_topic:
            break
        rail_index+=1
    ...
    try:
        # print(store_object.Measurement_float[rail_index][1][-1])
        # print(abs(voltage-store_object.Measurement_float[rail_index][1][-1]))
        # print(abs(current-store_object.Measurement_float[rail_index][2][-1]))
        store_object.quantisierung[rail_index][0].append(voltage-store_object.Measurement_float[rail_index][1][-1])
        store_object.quantisierung[rail_index][1].append(current-store_object.Measurement_float[rail_index][2][-1])
        store_object.time[rail_index].append(timestamp -store_object.Measurement_float[rail_index][0][0])
    except:
        len(store_object.Measurement_float[rail_index][1]) < 2


 
    store_object.Measurement_float[rail_index][0].append(timestamp)
    store_object.Measurement_float[rail_index][1].append(voltage)                                                  
    store_object.Measurement_float[rail_index][2].append(current)                                                  
    store_object.Measurement_float[rail_index][3].append(power)                                                       
    # print(store_object.Measurement_float[0][0])     
    ...
    # DATA_CHUNKING = 1
    # # slice data for 6 diffrent values in one package
    # sliced_data = chunks(hex_value, 6)
